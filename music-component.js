...
class MusicComponent extends PolymerElement {
  static get template() {
    return html`
      ...
      paper-slider{
        width: 100%;
        --paper-slider-active-color:var(--music-slider-color);
        --paper-slider-knob-color:var(--music-slider-knob-color);
      }
    ...
    <iron-ajax id="musicRequest" method="get" url="[[listRequestUrl]]" handle-as="json" on-response="songsRecovered" on-error="errorSongs">
    </iron-ajax>
    ...
    <div class="listContainer">
        <dom-repeat items="[[songsList]]" as="song" filter="{{filterSongs(searchArgument)}}">
          <template>
  ...
  connectedCallback() {
    ...
    this.$.musicRequest.generateRequest();
  }
  ...
  selectSong(event) {
    this.set("trackUrl", "");
    this.set("trackUrl", `${this.songUrl}${event.model.song}`);
    this.set("songName", event.model.song);
    const musicPlayer = this.shadowRoot.querySelector("#musicPlayer");
    musicPlayer.load();
    musicPlayer.addEventListener("loadedmetadata", (event) => {
      this.set("songDuration", musicPlayer.duration)
  ...
  songsRecovered(event){
    const payload = this.$.musicRequest.lastResponse;
    this.set("songsList", payload.tracks);
